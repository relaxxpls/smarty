...
def select_obj(msg):
    ...
    if int(key) == 0:
        if len(contour) > 0:
            ...
            smarty_data.area_default = cv2.contourArea(largest_contour)
    ...
    pub = rospy.Publisher('smarty_data', smarty_msgs, queue_size=10)
    pub.publish(smarty_data)
    cv2.waitKey(10)

def main():
    rospy.Subscriber("/smarty_camera/image_raw", Image, select_obj)
    try:
        rospy.spin()
